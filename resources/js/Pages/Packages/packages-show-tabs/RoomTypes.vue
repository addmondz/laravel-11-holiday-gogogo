    ...
    <div class="space-y-6" style="position: relative;">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <draggable
                    v-if="roomTypesData?.data?.length > 0"
                    v-model="roomTypesData.data"
                    tag="tbody"
                    item-key="id"
                    handle=".drag-handle"
                    :component-data="{ class: 'bg-white divide-y divide-gray-200' }"
                    @end="onDragEnd"
                >
                    <template #item="{ element: roomType }">
                        <tr>
                            <td class="px-2 py-4 whitespace-nowrap w-10">
                                <div class="drag-handle cursor-grab active:cursor-grabbing text-gray-400 hover:text-gray-600 flex justify-center">
                ...
                <tbody v-else class="bg-white divide-y divide-gray-200">
                    <tr>
                        <td colspan="10" class="text-center text-gray-500 py-4 border-t border-b border-gray-300">No room types found</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
const onDragEnd = async () => {
    const orderedIds = roomTypesData.value.data.map(rt => rt.id);
    try {
        await axios.post(route('room-types.update-sequence'), {
            ordered_ids: orderedIds,
        });
    } catch (error) {
        console.error('Failed to update sequence:', error);
        // Revert by re-fetching
        handlePageChange(roomTypesData.value.current_page || 1);
    }
};
