<template>
    <Head title="Edit Configuration Prices" />
    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                Edit Configuration Prices
            </h2>
        </template>
...
onMounted(() => {
    if (props.prices) {
        Object.entries(props.prices).forEach(([type, prices]) => {
            prices.forEach(price => {
                const index = getPriceIndex(price.number_of_adults, price.number_of_children);
                form.prices[index].adult_price = price.adult_price;
                form.prices[index].child_price = price.child_price;
            });
        });
    }
});
...
const submit = () => {
    form.put(route('configuration-prices.update', props.configuration.id), {
        onSuccess: () => {
